...
void f_meas_imp_field( int npbp_reps, quark_invert_control *qic, Real mass,
...
#if 1
   /************************************************************************
   *  Here we put in the code for generating perm and hperm for the 
   *  hierarchical coloring vectors. This code is adapted from Arjun 
   *  Singh Gambhir's code in LaLiBe and adapted for use in MILC_QCD by 
   *  Heather Switzer
   ***********************************************************************/

   #define starting_vector 0
   #define ending_vector 31

  /* Create the HP stuff, adopted from Andreas' main function */
   struct meshVars mesh;
   unsigned int j, N, Hpsize, sample;
   unsigned int *perm, *Hperm;

   mesh.d = 4; // Number of dimensions
   mesh.ptsPerDim = (unsigned int *)malloc(sizeof(unsigned int)*mesh.d);
   N = 1;
   
   unsigned int lattSize[4] = {nx, ny, nz, nt};
   // All dimensions of the lattice must be a power of 2 to do hierarchical
   // probing. Increase size of lattice to next power of 2.
   for(j = 0; j < mesh.d; ++j){
        // Compute the next highest power of 2 of 32-bit v
        unsigned int v = lattSize[j];
        v--;
        v |= v >> 1;
        v |= v >> 2;
        v |= v >> 4;
   ...
   */
   ...
   for(jpbp_reps = 0; jpbp_reps < npbp_reps; jpbp_reps++){
      for(colorSource = 0; colorSource < 3; colorSource++){
         //Loop over color from 0-2
         rfaction = (double)0.0;
...
#endif
#if 1
         HPsource_plain_field( gr, colorSource, perm, Hperm);
#else
